"""Verify the THREEPHOTON_LIBRARY_DIR env-var override redirects
the catalog away from the installed user's ``~/.3photon/library``.
...
def test_env_override_redirects(monkeypatch):
    """Setting THREEPHOTON_LIBRARY_DIR points library_dir at the override."""
    with tempfile.TemporaryDirectory(prefix="3photon_override_") as d:
        monkeypatch.setenv("THREEPHOTON_LIBRARY_DIR", d)
...
def test_env_override_expanduser(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("THREEPHOTON_LIBRARY_DIR", "~/my-test-library")
    resolved = library_paths.library_dir()
...
def test_env_override_unset_uses_default(monkeypatch):
    ...
    monkeypatch.delenv("THREEPHOTON_LIBRARY_DIR", raising=False)
    resolved = library_paths.library_dir()
    # Default path lives under ``.3photon/library`` somewhere under the user's home.
    assert resolved.endswith(os.path.join(".3photon", "library"))


def test_env_override_blank_uses_default(monkeypatch):
    """An empty string env var is treated as 'unset', not as 'override to root'."""
    monkeypatch.setenv("THREEPHOTON_LIBRARY_DIR", "")
    resolved = library_paths.library_dir()
    assert resolved.endswith(os.path.join(".3photon", "library"))
