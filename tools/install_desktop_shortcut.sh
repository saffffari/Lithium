#!/usr/bin/env bash
# Install a Linux desktop launcher for Lithium (app menu + ~/Desktop).
...
if [ -d "$DESK" ]; then
  cp "$DEST" "$DESK/lithium.desktop"
  chmod +x "$DESK/lithium.desktop"
  # KDE/GNOME: mark the desktop file as trusted so it launches on click.
  gio set "$DESK/lithium.desktop" metadata::trusted true 2>/dev/null || true
  echo "installed: $DESK/lithium.desktop"
fi

update-desktop-database "$APPS" 2>/dev/null || true
echo "done — Lithium should appear in your application launcher."
